defmodule Mosaic.HealthCheck do
  ...
  @spec get_metric(metric_name()) :: integer() | nil
  ...
  defp check_router_health do
    try do
      ...
      case Mosaic.ShardRouter.find_similar_shards_sync(test_vector, 1, [use_cache: true]) do
        shards when is_list(shards) -> {:router, :ok}
        _ -> {:router, :ok}
      end
    rescue
      _ -> {:router, :failed}
    catch
      :exit, _ -> {:router, :failed}
    end
  end
  ...
  defp check_embedding_service do
    try do
      task = Task.async(fn ->
        GenServer.call(Mosaic.EmbeddingService, {:encode, "health-check-test"}, 5000)
      end)
