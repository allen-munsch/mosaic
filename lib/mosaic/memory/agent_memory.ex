defmodule Mosaic.Memory.AgentMemory do
  ...
  def stats(session_id) when is_binary(session_id) do
    with {:ok, conn} <- get_memory_conn() do
      ...
      {:ok, [[consolidated]]} = Mosaic.DB.query(conn,
        "SELECT COUNT(*) FROM memories WHERE session_id = ? AND consolidation_of IS NOT NULL", [session_id])
  ...
  def query_memories(query_text, session_id, opts) do
    ...
    case result do
      ...
      {:ok, {:error, reason}} ->
        ...
        []
      {:error, reason} ->
        Logger.warning("Memory connection failed: #{inspect(reason)}")
        []
    end
  end
  ...
  defp rank_memories(memories, _query) do
  ...
  @doc "Ensure the memory database schema exists."
  def ensure_schema do
    path = memory_db_path()
    last_path = Process.get(:memory_schema_ensured_for)
    unless last_path == path do
      Process.put(:memory_schema_ensured_for, path)
