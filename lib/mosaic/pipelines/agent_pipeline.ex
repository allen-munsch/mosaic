defmodule Mosaic.Pipelines.AgentPipeline do
  ...
  defp execute_step(:traverse, config, results, _params) do
    _relation = config[:relation] || "callees"
  ...
  defp persist_pipeline(pipeline) do
    with {:ok, conn} <- get_conn() do
      # Convert steps from tuples to JSON-encodable maps
      steps_json = Enum.map(pipeline.steps, fn {type, config} ->
        # Convert keyword list config to map for JSON encoding
        config_map = if is_list(config) and Keyword.keyword?(config) do
          Map.new(config)
        else
          config
        end
        %{"type" => Atom.to_string(type), "config" => config_map}
      end)
      ...
      """, [
        ...
        Jason.encode!(steps_json), Jason.encode!(pipeline.tags),
  ...
  @doc "Ensure the pipeline database schema exists."
  def ensure_schema do
    path = pipeline_db_path()
    last_path = Process.get(:pipeline_schema_ensured_for)
    unless last_path == path do
      Process.put(:pipeline_schema_ensured_for, path)
      ...
      {:ok, list} when is_list(list) ->
        Enum.map(list, fn
          %{"type" => type, "config" => config} ->
            # Convert config map back to keyword list for step execution
            config_kw = if is_map(config), do: Enum.map(config, fn {k, v} -> {String.to_atom(k), v} end), else: config
            {String.to_atom(type), config_kw}
          [type, config] ->
            {String.to_atom(type), config}
          _ -> nil
        end)
