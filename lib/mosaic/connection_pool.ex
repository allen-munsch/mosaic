defmodule Mosaic.ConnectionPool do
  ...
  @type t :: %__MODULE__{pools: %{String.t() => [reference()]}, config: map()}
  @type conn :: reference()
  ...
  @spec checkout(shard_path()) :: {:ok, conn()} | {:error, term()}
  def checkout(shard_path), do: GenServer.call(__MODULE__, {:checkout, shard_path})

  @spec checkin(shard_path(), conn()) :: :ok
  def checkin(shard_path, conn), do: GenServer.cast(__MODULE__, {:checkin, shard_path, conn})

  @doc "Check out a connection, run the given function, then automatically check it back in."
  @spec scoped_checkout(shard_path(), (conn() -> result)) :: {:ok, result} | {:error, term()} when result: term()
  def scoped_checkout(shard_path, fun) do
    case checkout(shard_path) do
      {:ok, conn} ->
        result =
          try do
            fun.(conn)
