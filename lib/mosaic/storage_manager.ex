defmodule Mosaic.StorageManager do
  ...
  def handle_call({:create_shard, path}, _from, state) do
    ...
    try do
      ...
      # Sync to remote storage backend if enabled
      Mosaic.ShardSync.push_shard(path)
...
defp sqlite_vec_path do
  System.get_env("SQLITE_VEC_PATH") ||
    find_vec_extension([
      # Release path (Application.app_dir for sqlite_vec dependency)
      Application.app_dir(:sqlite_vec, "priv/0.1.5/vec0.so"),
      # Dev/build paths
      "priv/sqlite_vec/vec0.so",
      "_build/prod/lib/sqlite_vec/priv/0.1.5/vec0.so",
      "deps/sqlite_vec/priv/0.1.5/vec0.so"
    ])
end

defp find_vec_extension(paths) do
  Enum.find_value(paths, fn path ->
    abs = Path.expand(path, File.cwd!())
    if File.exists?(abs), do: abs
  end)
end
