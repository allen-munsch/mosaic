defmodule Mosaic.Auth.APIKey do
  ...
  @type claims :: %{sub: String.t(), scopes: [String.t()], tenant_id: String.t(), auth_method: String.t()}

  @doc "Create a new API key for a tenant with specified scopes."
  @spec create_key(tenant_id(), scopes(), keyword()) :: {:ok, api_key(), key_id()} | {:error, term()}
  ...
  def init_auth_db do
    ...
    # Also initialize system DB used by tenancy isolator
    system_db = Path.join(Mosaic.Tenancy.Isolator.system_path(), "system.db")
    if system_db != db_path do
      create_schema(system_db)
    end
  end

  defp create_schema(db_path) do
    ...
    # Open directly without ConnectionPool to avoid vec extension dependency
