defmodule Mosaic.Graph.Writer do
  ...
  # ── Convenience wrappers (auto-manage connection) ────────────

  @doc "Write nodes with automatic connection management."
  def write_nodes(nodes) when is_list(nodes) do
    shard_path = Mosaic.Config.get(:storage_path)
    with {:ok, conn} <- Mosaic.ConnectionPool.checkout(shard_path) do
      result = write_nodes(conn, nodes)
      Mosaic.ConnectionPool.checkin(shard_path, conn)
      result
    end
  end

  @doc "Write edges with automatic connection management."
  def write_edges(edges) when is_list(edges) do
    shard_path = Mosaic.Config.get(:storage_path)
    with {:ok, conn} <- Mosaic.ConnectionPool.checkout(shard_path) do
      result = write_edges(conn, edges)
      Mosaic.ConnectionPool.checkin(shard_path, conn)
      result
    end
  end

  @doc "Write a subgraph with automatic connection management."
  def write_subgraph(nodes, edges) do
    shard_path = Mosaic.Config.get(:storage_path)
    write_subgraph(shard_path, nodes, edges)
  end
end
